...
class CategoriesItemDecoration(context: Context) : RecyclerView.ItemDecoration() {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State?) {
        ...
        for (i in 0..parent.childCount - 1) {
            ...
            if (position < state!!.itemCount - 1) {
                c.drawLine(child.left.toFloat(), (child.bottom + offset).toFloat(), 0f, (child.bottom + offset).toFloat(), mWhitePaint)
                c.drawLine(0f, (child.bottom + offset).toFloat(), child.right.toFloat(), (child.bottom + offset).toFloat(), mDividerPaint)
